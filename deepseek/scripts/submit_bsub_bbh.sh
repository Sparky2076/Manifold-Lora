...
for v in TASKS NUM_FEWSHOT EVAL_BATCH_SIZE EVAL_LIMIT MODEL_NAME TORCH_DTYPE TRUST_REMOTE_CODE OUTPUT_JSON CONDA_ROOT CONDA_BASE CONDA_ENV_NAME; do
  eval "export $v=\"\${$v:-}\""
done

RUN_CMD="bash $SCRIPT_DIR/run_deepseek_bbh_bsub.sh"
for v in TASKS NUM_FEWSHOT EVAL_BATCH_SIZE EVAL_LIMIT MODEL_NAME TORCH_DTYPE TRUST_REMOTE_CODE OUTPUT_JSON METRICS_DIR ADAPTER_PATH CONDA_ROOT CONDA_BASE CONDA_ENV_NAME; do
  eval "val=\${$v}"
  if [[ -n "${val:-}" ]]; then
