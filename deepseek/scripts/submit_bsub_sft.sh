...
for v in EPOCHS MAX_STEPS TORCH_DTYPE GRADIENT_CHECKPOINTING BATCH_SIZE GRAD_ACCUM_STEPS MAX_LENGTH LR LORA_TYPE LORA_R LORA_ALPHA LORA_DROPOUT SFT_PRESET SFT_VAL_RATIO SFT_DATASET SFT_SPLIT; do
  eval "val=\${$v}"
  if [[ -n "${val:-}" ]]; then
