#!/usr/bin/env bash
# Qwen3 mLoRA refine → results_mmlu_mlora_refine/ （alpha=2r）
set -euo pipefail
...
export GRID_JOB_PREFIX="${GRID_JOB_PREFIX:-qwen3_mmlu_mlora_refine}"
export LORA_TYPE="${LORA_TYPE:-mlora}"
export RESULTS_ROOT="${RESULTS_ROOT:-$PROJECT_DIR/qwen3_autogrid/results_mmlu_mlora_refine}"
export GRID_PID_FILE="${GRID_PID_FILE:-$PROJECT_DIR/qwen3_autogrid/.grid_submitter_mlora_refine.pid}"
