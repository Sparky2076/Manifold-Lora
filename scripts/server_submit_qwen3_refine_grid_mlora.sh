#!/usr/bin/env bash
# Qwen3 细网格 mLoRA（48 jobs → results_mmlu_mlora_refine, alpha=2r）
set -euo pipefail
...
echo "==> MODEL_NAME=$MODEL_NAME LORA_TYPE=$LORA_TYPE EXCLUDE_HOSTS=$EXCLUDE_HOSTS"
unset MAX_STEPS EVAL_EVERY BATCH_SIZE GRAD_ACCUM_STEPS MAX_LENGTH
exec bash "$PROJECT_DIR/qwen3_autogrid/run_mlora_refine_grid_bsub.sh"
